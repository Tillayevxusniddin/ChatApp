    ...
    <>
      <div className='flex items-center justify-between relative'>
        <div className='flex flex-col'>
          ...
          <p className='font-spaceGrotesk text-muted-foreground text-xs'>{getSoundLabel(session?.currentUser?.notificationSound)}</p>
        </div>
        ...
        <Popover open={isNotification} onOpenChange={setIsNotification}>
          ...
          <PopoverContent className='w-80 absolute -right-12'>
            <div className='flex flex-col space-y-0'>
              {SOUNDS.map(sound => (
                <div className={cn('flex justify-between items-center bg-secondary cursor-pointer hover:bg-primary-foreground', selectedSound === sound.value && 'bg-primary-foreground')} 
                key={sound.label}
            ...
            <Button className='w-full mt-2 font-bold' disabled={isPending} onClick={() => mutate({notificationSound: selectedSound})}>
              Submit
            </Button>
          </PopoverContent>
        </Popover>
      ...
      <div className='flex items-center justify-between relative'>
        ...
        <Popover open={isSounding} onOpenChange={setIsSounding}>
          ...
          <PopoverContent className='w-80 absolute -right-12'>
            ...
            <Button className='w-full mt-2 font-bold' disabled={isPending} onClick={ () => mutate({ sendingSound: selectedSound})}>
              Submit
            </Button>
          </PopoverContent>
        </Popover>
